...
async function fetchCardsForSet(setId, pageSize = 30) {
  try {
    const query = encodeURIComponent(`set.id:${setId}`)
    const select = encodeURIComponent('id,name,number,rarity,set,images,tcgplayer')
    const data = await safeJson(`${pokemonApiBase}/cards?q=${query}&pageSize=${pageSize}&orderBy=-set.releaseDate&select=${select}`)
    return (data.data || []).filter((card) => card?.tcgplayer?.prices)
  } catch {
...
function pickTopCandidates(cards, max = 6) {
  return cards
    ...
    .filter((card) => card.model.pricing)
    .sort((a, b) => b.model.score - a.model.score || (b.model.pricing.market || 0) - (a.model.pricing.market || 0))
    .slice(0, max)
}
...
app.get('/api/card-search', async (req, res) => {
  ...
  try {
    ...
    const select = encodeURIComponent('id,name,number,rarity,set,images,tcgplayer')
    const data = await safeJson(`${pokemonApiBase}/cards?q=${query}&pageSize=20&orderBy=-set.releaseDate&select=${select}`)
...
app.get('/api/predict', async (req, res) => {
  ...
  if (term) {
    try {
      const query = encodeURIComponent(`name:"${term}" OR name:${term}`)
      const select = encodeURIComponent('id,name,number,rarity,set,images,tcgplayer')
      const data = await safeJson(`${pokemonApiBase}/cards?q=${query}&pageSize=10&orderBy=-set.releaseDate&select=${select}`)
      card = (data.data || []).find((entry) => entry?.tcgplayer?.prices) || null
    } catch {
  ...
  if (!card) {
    return res.status(404).json({ error: 'No card with pricing data matched that search.' })
  }
